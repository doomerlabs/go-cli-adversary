#!/usr/bin/env node
import { createRequire as __goCliCreateRequire } from 'node:module'; const require = __goCliCreateRequire(import.meta.url);
...
var GO_INTEGER = "(?:0[bB]_?[01](?:_?[01])*|0[oO]_?[0-7](?:_?[0-7])*|0[xX]_?[0-9a-fA-F](?:_?[0-9a-fA-F])*|0(?:_?[0-7])+|(?:0|[1-9](?:_?\\d)*))";
function cobraPositionalArgsMinimumSignals(file, root) {
  ...
  for (const literal of root.descendantsOfType("composite_literal")) {
    ...
    for (const access of positionalAccesses(callback, argsName)) {
      ...
      const unwrappedCallbackValue = callbackValue === void 0 ? void 0 : unwrap(callbackValue);
      const callbackBinding = unwrappedCallbackValue?.type === "identifier" ? unwrappedCallbackValue : void 0;
      const evidence = [callbackBinding, validator, ...assignments, ...relevantGuardConditions(access.node, callback, argsName)].filter((node) => node !== void 0);
...
function minimumProvenByValidator(validator, declarations, cobraAliases, nonNilErrorConstructors) {
  ...
  if (value.type === "identifier") {
    if (isNameShadowedAt(validator, value.text)) return void 0;
...
function returnsNonNil(block, nonNilErrorConstructors) {
  ...
  if (constructor === void 0 || !nonNilErrorConstructors.has(constructor)) return false;
...
function isImmediatelyExecutedClosure(owner, callback) {
  return closureInvocation(owner, callback) !== void 0;
}
function closureInvocation(owner, callback) {
  if (owner.type !== "func_literal") return void 0;
  for (let current = owner.parent; current !== null && !current.equals(callback); current = current.parent) {
    if (["parenthesized_expression", "literal_element"].includes(current.type)) continue;
    if (current.type !== "call_expression") return void 0;
    const fn = current.childForFieldName("function");
    if (fn === null || !contains(fn, owner)) return void 0;
...
function integerLiteral(node) {
  if (node?.type !== "int_literal") return void 0;
  return parseGoInteger(node.text);
}
function parseGoInteger(literal) {
  const text2 = literal.replaceAll("_", "");
  let value;
  if (/^0[bB][01]+$/.test(text2)) value = Number.parseInt(text2.slice(2), 2);
  else if (/^0[oO][0-7]+$/.test(text2)) value = Number.parseInt(text2.slice(2), 8);
  else if (/^0[xX][0-9a-fA-F]+$/.test(text2)) value = Number.parseInt(text2.slice(2), 16);
  else if (/^0[0-7]+$/.test(text2) && text2.length > 1) value = Number.parseInt(text2.slice(1), 8);
  else if (/^(?:0|[1-9]\d*)$/.test(text2)) value = Number.parseInt(text2, 10);
  else return void 0;
...
function minimumProvenAtAccess(access, callback, argsName, proofAfterIndex) {
  ...
  for (let current = access; current !== null && !current.equals(callback); current = current.parent) {
    ...
    if (parent?.type === "for_statement") {
      ...
      if (condition !== void 0 && condition.endIndex > proofAfterIndex && body2 !== null && contains(body2, access)) {
        minimum = Math.max(minimum, acceptedMinimum(condition, argsName) ?? 0);
      }
    }
    if (parent?.type === "expression_case") {
      ...
      if (switchNode?.type === "expression_switch_statement" && expression !== null && expression !== void 0 && expression.endIndex > proofAfterIndex && isUnshadowedLenOf(expression, argsName)) {
...
function forCondition(statement) {
  const clause = statement.namedChildren.find((child) => child.type === "for_clause");
  if (clause !== void 0) return clause.childForFieldName("condition") ?? void 0;
...
function controlHeaderDeclares(node, reference, name2) {
  ...
  if (node.type === "communication_case") {
    ...
    return communication?.type === "receive_statement" && communication.text.includes(":=") && expressionListContainsName(communication.childForFieldName("left"), name2);
  }
  return false;
}
...
function minimumAfterAssignments(access, callback, argsName, initialMinimum) {
  ...
  return {
    ...
    proofAfterIndex: nodes.filter((node) => node.startIndex < access.startIndex).reduce((latest, node) => Math.max(latest, node.endIndex), -1)
  };
}
...
function priorAssignments(access, callback, argsName) {
  ...
  const delayedInvocation = accessOwner === null || accessOwner.equals(callback) ? void 0 : delayedClosureInvocation(accessOwner, callback);
  const sorted = candidates.filter((node) => {
    const owner = nearestFunction(node);
    const ownerInvocation = owner === null || owner.equals(callback) ? void 0 : closureInvocation(owner, callback);
    if (owner === null || !owner.equals(callback) && ownerInvocation === void 0 || accessOwner?.equals(callback) === true && ownerInvocation?.kind === "defer") return false;
    const lexicallyPrior = node.startIndex < access.startIndex;
    const laterOuterWrite = delayedInvocation !== void 0 && owner.equals(callback) && node.startIndex > delayedInvocation.endIndex;
  ...
  return sorted.filter((assignment, index) => !assignmentIsNoOp(assignment, argsName) && !sorted.slice(index + 1).some((later) => unconditionallyOverwrites(assignment, later)));
}
function assignmentIsNoOp(assignment, argsName) {
  return assignedValue(assignment, argsName)?.text === argsName;
}
function delayedClosureInvocation(owner, callback) {
  const invocation = closureInvocation(owner, callback);
  return invocation?.kind === "defer" || invocation?.kind === "go" ? invocation.node : void 0;
}
function unconditionallyOverwrites(assignment, later) {
  return assignment.type === "assignment_statement" && later.type === "assignment_statement" && assignment.parent?.type === "statement_list" && later.parent?.equals(assignment.parent) === true;
}
function canReachAccessAfter(node, access, callback) {
  for (let current = node.parent; current !== null && !current.equals(callback); current = current.parent) {
    ...
    if (current.type === "communication_case" && current.parent?.type === "select_statement" && contains(current.parent, access)) return false;
    if (["expression_case", "type_case", "default_case"].includes(current.type) && current.parent !== null && contains(current.parent, access) && !caseFallsThroughTo(current, access)) return false;
...
function caseFallsThroughTo(source, access) {
...
function assignedMinimum(assignment, argsName, currentMinimum) {
  ...
  if (value?.type === "call_expression" && value.childForFieldName("function")?.text === "make" && !builtinIsShadowed(value, "make")) {
    const args2 = value.childForFieldName("arguments")?.namedChildren ?? [];
    if (args2[0]?.text === "[]string") return integerLiteral(args2[1]);
  }
  if (value?.type === "call_expression" && value.childForFieldName("function")?.text === "append" && !builtinIsShadowed(value, "append")) {
    const args2 = value.childForFieldName("arguments")?.namedChildren ?? [];
    if (args2.length === 0) return void 0;
    const fixedAppends = args2.slice(1).filter((item) => item.type !== "variadic_argument").length;
    const preserved = args2[0]?.text === argsName ? currentMinimum : 0;
    return Math.max(preserved, fixedAppends);
  }
  return void 0;
}
function assignedValue(assignment, argsName) {
  if (assignment.type !== "assignment_statement" && assignment.type !== "receive_statement") return void 0;
...
function blockSkipsAccess(node, access) {
  if (node.type === "if_statement") {
    const consequence = node.childForFieldName("consequence");
    const alternative = node.childForFieldName("alternative");
    return consequence !== null && alternative !== null && blockSkipsAccess(consequence, access) && blockSkipsAccess(alternative, access);
  }
  const statements = statementList(node)?.namedChildren ?? node.namedChildren;
  const last = statements.at(-1);
  return last !== void 0 && terminalSkipsAccess(last, access);
}
function terminalSkipsAccess(statement, access) {
  ...
  if (statement.type === "expression_statement") {
    ...
    return expression?.type === "call_expression" && expression.childForFieldName("function")?.text === "panic" && !builtinIsShadowed(expression, "panic");
  }
  ...
  if (statement.type === "continue_statement") {
    ...
    const loop = nearestAncestor(statement, /* @__PURE__ */ new Set(["for_statement"]));
    return loop !== void 0 && contains(loop, access);
  }
  if (statement.type === "break_statement") {
    if (statement.text.trim() !== "break") return false;
    const target = nearestAncestor(statement, /* @__PURE__ */ new Set([
      "for_statement",
      "expression_switch_statement",
      "type_switch_statement",
      "select_statement"
    ]));
    return target !== void 0 && contains(target, access);
  }
  return false;
}
function exhaustiveSwitchSkipsAccess(statement, access) {
  const cases = statement.namedChildren.filter((child) => child.type === "expression_case" || child.type === "type_case" || child.type === "default_case");
...
function builtinIsShadowed(reference, name2) {
  if (isNameShadowedAt(reference, name2)) return true;
  let root = reference;
  while (root.parent !== null) root = root.parent;
  for (const declaration of root.namedChildren) {
    if (declaration.type === "function_declaration" && declaration.childForFieldName("name")?.text === name2) {
      return true;
    }
    if (["var_declaration", "const_declaration", "type_declaration"].includes(declaration.type) && declarationNames(declaration).has(name2)) return true;
  }
  return false;
}
...
function minimumAfterSwitch(statement, access, argsName) {
  const expression = switchExpression(statement);
  if (expression === void 0 || !isUnshadowedLenOf(expression, argsName)) return 0;
  const cases = statement.namedChildren.filter((child) => child.type === "expression_case" || child.type === "default_case");
  ...
  for (let value = 0; value <= max + 1; value += 1) {
    ...
    if (item === void 0 || !caseSkipsAccess(item, access)) return value;
  }
  return Number.MAX_SAFE_INTEGER;
}
function minimumFromEnclosingSwitchDefault(access, callback, argsName, proofAfterIndex) {
  for (let current = access; current !== null && !current.equals(callback); current = current.parent) {
    if (current.type !== "default_case") continue;
    const statement = current.parent;
    const expression = statement?.type === "expression_switch_statement" ? switchExpression(statement) : void 0;
    if (statement === null || statement === void 0 || expression === void 0 || expression.endIndex <= proofAfterIndex || !isUnshadowedLenOf(expression, argsName)) return 0;
    const covered = /* @__PURE__ */ new Set();
    for (const item of statement.namedChildren.filter((child) => child.type === "expression_case" && !child.equals(current))) {
      const values = switchCaseValues(item);
      if (values === void 0 || caseFallsThrough(item)) return 0;
...
function switchExpression(statement) {
  return statement.childForFieldName("value") ?? statement.childForFieldName("expression") ?? statement.namedChildren.find((child) => child.type !== "expression_case");
}
function isUnshadowedLenOf(expression, argsName) {
  return expression.text.replace(/\s/g, "") === `len(${argsName})` && !builtinIsShadowed(expression, "len");
}
function switchCaseValues(item) {
...
function rejectedMinimum(condition, argsName) {
  ...
  const text2 = normalizedCondition(condition.text);
  const len = escaped(`len(${argsName})`);
  const empty = text2.match(new RegExp(`^${len}==(${GO_INTEGER})$`));
...
function matchMinimum(text2, patterns) {
  for (const [pattern, increment, fixed] of patterns) {
    ...
    if (literal !== void 0) return literal + increment;
  }
  return void 0;
}
function normalizedCondition(text2) {
  let normalized = text2.replace(/\s/g, "");
...
var domain = {
  ...
  rules: [
    ...
    },
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} command path${count === 1 ? "" : "s"} terminate the process below the application boundary.`,
    ...
    },
    {
      ...
      severity: "medium",
      confidence: "high",
      summary: (count) => `${count} command path${count === 1 ? "" : "s"} start work from context.Background or context.TODO.`,
    ...
    },
    {
      ...
      severity: "medium",
      confidence: "high",
      summary: (count) => `${count} subprocess launch${count === 1 ? "" : "es"} use exec.Command without a parent context.`,
      whyItMatters: "Child processes started without CommandContext keep running after the CLI is cancelled or times out.",
      impact: "Orphaned git, curl, docker, and build processes continue after Ctrl-C or CI cancellation.",
      recommendation: "Use exec.CommandContext with the command context so children are killed or wait with cancellation."
    },
    {
      ...
      severity: "high",
      confidence: "medium",
      summary: (count) => `${count} command path${count === 1 ? "" : "s"} invoke a shell with -c to run a composed string.`,
    ...
    },
    {
      ...
      severity: "low",
      confidence: "medium",
      summary: (count) => `${count} package init block${count === 1 ? "" : "s"} construct clients or perform I/O.`,
    ...
    },
    {
      ...
      severity: "low",
      confidence: "medium",
      summary: (count) => `${count} non-main path${count === 1 ? "" : "s"} read os.Args instead of injected args.`,
    ...
    },
    {
      ...
      severity: "low",
      confidence: "medium",
      summary: (count) => `${count} path${count === 1 ? "" : "s"} emit spinner/ANSI sequences without an in-file TTY guard.`,
    ...
    },
    {
      ...
      severity: "medium",
      confidence: "medium",
      summary: (count) => `${count} path${count === 1 ? "" : "s"} pass variables into git/docker/kubectl argv after options without an obvious validator.`,
    ...
    },
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} flag definition${count === 1 ? "" : "s"} accept password/token/secret material on argv.`,
      whyItMatters: "Process argument vectors are visible to other local users via ps, audit logs, shell history, and crash reporters.",
      impact: "Credentials leak through process listings and shared-host observability without touching application logs.",
      recommendation: "Prefer env vars, files with restricted modes, or interactive prompts; if a secret flag remains, MarkHidden it and document env/file alternatives."
    },
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} update/download path${count === 1 ? "" : "s"} use plain HTTP or disable TLS verification.`,
      whyItMatters: "Self-update binaries and release assets must be integrity-protected; plain HTTP or skip-verify enables MITM code execution.",
      impact: "An active network attacker can substitute a malicious binary during CLI update or install.",
      recommendation: "Download over HTTPS only, verify checksums/signatures, and never set InsecureSkipVerify on update transports."
    }
  ],
...
function executeErrorSignals(file) {
  if (isNonProductPath(file.path)) return [];
  return lineSignals(
    file,
    "go-cli.execute-error",
    /^\s*(?:_\s*=\s*)?(?:rootCmd|cmd|app)\.Execute(?:Context)?\(\)\s*$/,
    (match) => match[0]?.includes("_") ? "The command execution error is discarded with a blank identifier." : "The command execution error is not inspected or returned."
  );
}
var CREDENTIAL_FLAG_NAME = /["'](password|passwd|secret|token|api-key|apikey|access-token|api_key)["']/i;
function flagsPasswordArgvSignals(file) {
  if (isNonProductPath(file.path)) return [];
  return lineSignals(
    file,
    ...
    (match) => {
      const name2 = match[0]?.match(CREDENTIAL_FLAG_NAME)?.[1] ?? "credential";
      return `Flag "${name2}" accepts secret material on argv (visible in process listings).`;
    }
  ).filter((signal) => {
    const name2 = (file.current.split("\n")[signal.line - 1] ?? "").match(CREDENTIAL_FLAG_NAME)?.[1];
    if (name2 === void 0) return false;
    const hidden = new RegExp(
      String.raw`MarkHidden\s*\(\s*["']${name2.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}["']`,
      "i"
    );
    return !hidden.test(file.current);
  });
}
function updateInsecureSignals(file) {
  if (isNonProductPath(file.path)) return [];
  ...
  const updatePath = /\b(?:self[-_]?update|autoUpdate|download(?:Binary|Update|Release)|CheckForUpdate|ApplyUpdate|go-selfupdate|go-update)\b/i.test(
    file.current
...
function subprocessSignals(file) {
  ...
  return lineSignals(
    ...
    (signal) => !signal.snippet.includes("CommandContext") && !/"(?:\/bin\/)?(?:ba)?sh"\s*,\s*"-c"/.test(signal.snippet)
  );
}
...
function initSideEffectSignals(file) {
  if (isNonProductPath(file.path)) return [];
  ...
  for (const match of initBodies) {
    const body2 = match[1] ?? "";
    if (!/\b(?:http\.|sql\.Open|Dial|Must\(|os\.Exit|exec\.Command)/.test(body2)) continue;
...
function osArgsOutsideMainSignals(file) {
  if (isNonProductPath(file.path)) return [];
...
function ansiNoTtySignals(file) {
  if (isNonProductPath(file.path)) return [];
  if (/\b(?:term\.IsTerminal|isatty\.IsTerminal|IsTerminal\s*\()/.test(file.current)) {
    return [];
  }
...
function optionSmugglingSignals(file) {
  if (isNonProductPath(file.path)) return [];
...
13. Cancellation swallowed by best-effort error handling \u2014 a CLI passes its context to an operation but treats cancellation as an ordinary recoverable failure, then continues work or returns success.

For swallowed cancellation, trace the command context through the operation, error handler, and reachable continuation. Report only when prepared source proves the operation can return cancellation of the command context, a changed warning/fallback/retry branch discards that cancellation, and subsequent command work or a successful exit remains reachable without an intervening cancellation check. Passing cmd.Context() alone does not establish correct cancellation handling. Cite the swallowing branch and the downstream continuation or success boundary together. Recommend propagating command cancellation before applying ordinary best-effort recovery, preserving wrapped cancellation errors where applicable.
Stay quiet when cancellation is propagated (including wrapped errors), the caller checks the command context before continuing or succeeding, or the downstream boundary is proven to stop immediately on cancellation. A deliberately recoverable child-operation timeout is not cancellation of the parent command: do not require aborting a still-live parent just because errors.Is(err, context.DeadlineExceeded) is true. Do not flag logging followed by an error return, required bounded cleanup after cancellation, or a possible cancellation path whose source or consequence is absent from the prepared evidence.
...
- Execution accounting across CLI orchestration: distinguish selected/planned items, started attempts, and successfully completed work according to the reporting contract. When a changed CLI reports usage or a completion summary from a runner result, trace the result population, callback/error exits, actual execution boundary, and reporting sink. Report only when prepared evidence proves the sink claims a specific execution stage, a reachable early callback/rendering/work failure returns items that have not reached that stage, and the caller still reports those items without filtering or a success guard. Cite the reporting contract, premature result population/exit, and caller emission together. A dry-run guard alone does not prove every selected item ran. Treat this as command-behavior, not a generic telemetry style concern.
Stay quiet when metrics explicitly count selections or attempts and the items reached that stage, result entries are appended only after the claimed stage, the caller filters by proven per-item lifecycle status, an error guard prevents the incorrect emission, or the relevant producer/sink semantics are absent. A failed command may still have legitimate partial completed usage; do not recommend dropping all usage on every error or counting only successes when the contract measures starts. Recommend recording the actual lifecycle stage per item and reporting the proven subset, preserving partial execution and avoiding duplicate reporting. Names such as Run, Selected, or reportUsage alone are not proof of semantics.
...
async function reviewDomain(ctx, analysis, discoveryFiles = []) {
  ...
  const ranked = [...candidates].sort(
    (left, right) => RISK_ORDER[right.rule.severity] - RISK_ORDER[left.rule.severity] || right.total - left.total || left.rule.id.localeCompare(right.rule.id)
  );
...
function assessmentSummary(active) {
  ...
  const ordered = active.slice(0, MAX_FINDINGS).map((item, index) => `${index + 1}) ${item.rule.concern} (${siteLabel(item.total)})`).join("; ");
  return `${active.length} priority lifecycle issues \u2014 address in order: ${ordered}.`;
}
function siteLabel(count) {
  return count === 1 ? "1 site" : `${count} sites`;
}
function addPositives(ctx, analysis, activeRuleIds) {
